...
# 
result := [P[1]];
result_fixed := ShallowCopy(result);
# 
...
# 
  local j, what_check_was, ii;
#   
#   point = choose_candidate(P_possible);
# 
  if i>Length(P_possible) then
    P_possible := P_possible_fixed;
    result := result_fixed;
    counter := counter+1;
    if counter>Length(P_possible) then
      # ???
    fi;
    i := counter;
  fi;
  ...
  while Length(result) < size_of_reguli do
