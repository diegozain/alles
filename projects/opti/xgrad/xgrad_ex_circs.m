...
clc
% ------------------------------------------------------------------------------
x=linspace(0,15,60); % (0,20,70)
z=linspace(0,15,60); % (0,10,35)
% ------------------------------------------------------------------------------
ar=0.5; % 1.5
br=1.5; % 0.5
ac=[fix(numel(z)*0.45),fix(numel(x)*0.5)];
bc=[fix(numel(z)*0.5),fix(numel(x)*0.5)];
a = gaussi(x,z,ac,ar,2);
b = (1-gaussi(x,z,bc,br,2));
% ------------------------------------------------------------------------------
...
circ_bx = 1.5*br*circ_x+x(bc(2));
circ_bz = 1.5*br*circ_z+z(bc(1));
% ------------------------------------------------------------------------------
% compute xgradient
[nz,nx] = size(a);
[Dz,Dx] = Dx_Dz(nz,nx);
...
plot(circ_ax,circ_az,'k-','linewidth',2)
plot(circ_bx,circ_bz,'k-','linewidth',2)
hold off
axis ij
% colormap(rainbow())
set(gca,'YTick',[])
