...
integer, parameter :: nt = 1024 ! 4096 ! a power of 2 so fft plays nice
...
! omp num of threads
integer, parameter :: nthreads=100
! omp chronometer
...
!$omp parallel private(iu,u,a,b,ddu,utap,rayleigh,reray,imray,absray,vx_num,vz_num,it,omegau,irx,bess_arg,bess_o,bess_1,Ivx,Ivz)

! start parallel block
!$omp do reduction(+:vx_rx,vz_rx) schedule(static,1)
! ------------------------------------------------------------------------------
do iu=0,nu
  u = dble(iu)*du+umin
  ...
  do it=iminf,imaxf
    ...
    do irx=1,nrx
      ...
      Ivx = bess_1 * (vx_num/rayleigh*utap*ddu)
      Ivz = bess_o * (vz_num/rayleigh*utap*ddu)

      vx_rx(it,irx) = vx_rx(it,irx) + Ivx
      vz_rx(it,irx) = vz_rx(it,irx) + Ivz
    end do ! recs
  end do ! freqs
end do ! slowness
! ------------------------------------------------------------------------------
